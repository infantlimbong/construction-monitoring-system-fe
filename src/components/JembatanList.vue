<template>
    <div class="px-5">
        <div v-if="showModal" class="absolute z-50 inset-0 flex justify-center items-center bg-black bg-opacity-70">
            <div :class="{ 'h-auto overflow-y-hidden': selectedLapKondisiJembatan.gambar_jembatan.length === 0 }" class="bg-main-black rounded-lg px-4 py-2 relative mx-4 h-5/6 lg:mx-8">
                <button @click="closeModal" class="absolute right-3 hover:text-gray-300">
                    <i class="bx bx-x-circle text-2xl"></i>
                </button>
                <div :class="{ 'mt-10 mb-3 flex justify-center flex-wrap items-center h-5/6 gap-y-6 gap-x-2 overflow-y-scroll': selectedLapKondisiJembatan.gambar_jembatan.length > 0 }" class="mt-10 mb-3">
                    <template v-if="selectedLapKondisiJembatan && selectedLapKondisiJembatan.gambar_jembatan">
                        <img v-for="(gambar, index) in selectedLapKondisiJembatan.gambar_jembatan.split(',')" :key="index" :src="getGambarUrl(gambar.trim())" alt="Gambar Jembatan" class="lg:h-80 h-40 md:h-60 object-cover rounded-lg mx-2">
                    </template>
                    <template v-else>
                        <p>Gambar kosong 💀</p>
                    </template>
                </div>
                <a :class="{ 'ms-5' : selectedLapKondisiJembatan.gambar_jembatan.length > 0 }" class="hover:underline text-sm text-blue-400" :href="generateGoogleMapsLink(selectedLapKondisiJembatan.koordinat_jembatan)" target="_blank">Link Koordinat</a>
            </div>
        </div>

        <h1 class="text-3xl my-5">List Jembatan</h1>
        <div class="flex flex-wrap gap-y-2 md:flex-nowrap md:justify-between">
            <div class="flex items-center gap-2">
                <router-link v-if="isAdmin || isKaryawan1 ||isKaryawan2" class="bg-blue-500 hover:bg-blue-700 text-md px-3 py-1 rounded transition-all duration-200" to="/createjembatan">Tambah</router-link>
            ...
            <div class="flex items-center justify-between w-full md:justify-normal md:gap-2 md:w-min">
                <input type="text" v-model="searchQuery" name="searchQuery" placeholder="Cari jembatan..." class="block py-1 px-3 border border-white rounded bg-main-black focus:outline-1">
        ...
        <div class="overflow-x-scroll">
            <table id="lap_kondisi_jembatanTable" class="min-w-full divide-y mt-2 divide-white">
                <thead>
                    <tr class="bg-stone-800">
                        ...
                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">
                            <div class="flex items-center">
                                Nama Jembatan
                                <button @click="sortByName('lap_kondisi_jembatan')" class="ml-1 block text-profile-color">
                ...
                <tbody class="divide-y divide-white" v-if="paginatedLapKondisiJembatan.length > 0">
                    <tr v-for="(lap_kondisi_jembatan, loop) in paginatedLapKondisiJembatan" :key="lap_kondisi_jembatan.id" class="odd:bg-main-black hover:bg-stone-950">
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ loop + 1 }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.nama_ruas_jembatan }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.no_ruas_jembatan }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.panjang_ruas_jembatan }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ getDesaName(lap_kondisi_jembatan.id_desa) }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.status_jembatan }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.kondisi_baik }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.kondisi_cukup_baik }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.kondisi_rusak_ringan }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.kondisi_rusak_berat }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.keseluruhan_kondisi }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ lap_kondisi_jembatan.tahun_pembangunan }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">
                            <div v-if="lap_kondisi_jembatan.progress > 0" class="w-full bg-gray-600 rounded">
                                <div v-if="lap_kondisi_jembatan.progress < 100" :style="{ width: lap_kondisi_jembatan.progress + '%' }" class="text-xs leading-none py-1 px-1 text-center text-white rounded" :class="{ 'rounded-r-none': lap_kondisi_jembatan.progress < 100, 'bg-red-500': lap_kondisi_jembatan.progress <= 25, 'bg-orange-500': lap_kondisi_jembatan.progress > 25 && lap_kondisi_jembatan.progress <= 50, 'bg-yellow-500': lap_kondisi_jembatan.progress > 50 && lap_kondisi_jembatan.progress <= 75, 'bg-lime-500': lap_kondisi_jembatan.progress > 75 && lap_kondisi_jembatan.progress < 100 }">
                                    {{ lap_kondisi_jembatan.progress }}%
                                </div>
                        ...
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">
                            <a class="hover:underline" :href="generateGoogleMapsLink(lap_kondisi_jembatan.koordinat_jembatan)" target="_blank">{{ lap_kondisi_jembatan.koordinat_jembatan }}</a>
                        </td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">
                            <button v-if="lap_kondisi_jembatan.gambar_jembatan.length > 0" @click="openModal(lap_kondisi_jembatan)" class="flex justify-center items-center border p-1 w-16 rounded-sm overflow-hidden">
                                <span v-for="(gambar, index) in lap_kondisi_jembatan.gambar_jembatan.split(',').slice(0, 2)" :key="index">
                                    <img :src="getGambarUrl(gambar.trim())" alt="" class="h-5 object-cover mr-2">
                                </span>
                            </button>
                            <button v-else @click="openModal(lap_kondisi_jembatan)" class="flex justify-center items-center text-xs text-gray-500 border p-1 w-16 rounded-sm overflow-hidden">
                                No image
                            </button>
                        </td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ formatDate(lap_kondisi_jembatan.created_at) }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ formatDate(lap_kondisi_jembatan.updated_at) }}</td>
                        <td v-if="showDetail" class="px-4 py-2 whitespace-nowrap text-sm">{{ getUserName(lap_kondisi_jembatan.id_user) }}</td>
                        <td v-if="isAdmin || isKaryawan1 || isKaryawan2" class="px-4 py-2 whitespace-nowrap text-sm">
                            <router-link :to="`/editjembatan/${lap_kondisi_jembatan.id}`" class="text-profile-color mr-3 hover:underline">
                                Edit
                            </router-link>
                            <button v-if="isAdmin || isKaryawan1" @click="deleteLapKondisiJembatan(lap_kondisi_jembatan)" class="text-red-500 hover:underline">
                                Hapus
                            </button>
                        </td>
                    </tr>
...
<script setup>
    ...
    const lap_kondisi_jembatans = ref([]);
    const desasMap = ref({});
    const usersMap = ref({});
    ...
    const selectedLapKondisiJembatan = ref(null);
    ...
    const uniqueYears = computed(() => {
        const years = new Set();
        lap_kondisi_jembatans.value.forEach(item => {
    ...
    const openModal = (lap_kondisi_jembatan) => {
        selectedLapKondisiJembatan.value = lap_kondisi_jembatan;
    ...
    const totalPages = computed(() => Math.ceil(filteredlap_kondisi_jembatans.value.length / entitiesPerPage.value));

    const fetchlap_kondisi_jembatans = async () => {
        const response = await axios.get("http://localhost:8000/api/lap_kondisi_jembatans", {
            headers: {
                Authorization: `Bearer ${authStore.accessToken}`,
            },
        });
        lap_kondisi_jembatans.value = response.data.data.reverse();
    ...
    onMounted(fetchlap_kondisi_jembatans);
    ...
    // Computed property to filter lap_kondisi_jembatans based on searchQuery
    const filteredlap_kondisi_jembatans = computed(() => {
        let filteredData = lap_kondisi_jembatans.value;
        
        if (selectedYear.value) {
            filteredData = filteredData.filter(item => item.tahun_pembangunan === selectedYear.value);
        }
        // Filter by searchQuery
        filteredData = filteredData.filter(lap_kondisi_jembatan => {
            const namaRuasJembatanMatches = lap_kondisi_jembatan.nama_ruas_jembatan.toLowerCase().includes(searchQuery.value.toLowerCase());
            const desaMatches = getDesaName(lap_kondisi_jembatan.id_desa).toLowerCase().includes(searchQuery.value.toLowerCase());

            return namaRuasJembatanMatches || desaMatches;
        });

        return filteredData;
    });

    // Computed property for paginated lap_kondisi_jembatans
    const paginatedLapKondisiJembatan = computed(() => {
        const startIndex = (currentPage.value - 1) * entitiesPerPage.value;
        const endIndex = startIndex + entitiesPerPage.value;
        return filteredlap_kondisi_jembatans.value.slice(startIndex, endIndex);
    });

    // Sweet alert for delete
    const deleteLapKondisiJembatan = async (lap_kondisi_jembatan) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`http://localhost:8000/api/lap_kondisi_jembatans/${lap_kondisi_jembatan.id}`, {
                    headers: {
                        Authorization: `Bearer ${authStore.accessToken}`,
                    },
                });
                ...
                await fetchlap_kondisi_jembatans();
            } catch (error) {
                console.error('Error deleting lap_kondisi_jembatan:', error);
                Swal.fire(
                    'Gagal menghapus!',
    ...
    const reverseOrder = () => {
        ...
        lap_kondisi_jembatans.value.reverse();
    };

    // Variables for sorting order
    let jembatanSortOrder = ref('asc');
    ...
    const sortByName = (type) => {
        if (type === 'lap_kondisi_jembatan') {
            if (jembatanSortOrder.value === 'asc') {
                lap_kondisi_jembatans.value.sort((a, b) => a.nama_ruas_jembatan.localeCompare(b.nama_ruas_jembatan));
                jembatanSortOrder.value = 'desc';
            } else {
                lap_kondisi_jembatans.value.sort((a, b) => b.nama_ruas_jembatan.localeCompare(a.nama_ruas_jembatan));
    ...
    const sortDesa = () => {
        if (desaSortOrder.value === 'asc') {
            lap_kondisi_jembatans.value.sort((a, b) => getDesaName(a.id_desa).localeCompare(getDesaName(b.id_desa)));
            desaSortOrder.value = 'desc';
        } else {
            lap_kondisi_jembatans.value.sort((a, b) => getDesaName(b.id_desa).localeCompare(getDesaName(a.id_desa)));
    ...
    const sortNomorRuas = () => {
        if (nomorRuasSortOrder.value === 'asc') {
            lap_kondisi_jembatans.value.sort((a, b) => a.no_ruas_jembatan.localeCompare(b.no_ruas_jembatan));
            nomorRuasSortOrder.value = 'desc';
        } else {
            lap_kondisi_jembatans.value.sort((a, b) => b.no_ruas_jembatan.localeCompare(a.no_ruas_jembatan));
    ...
    const sortPanjang = () => {
        if (panjangSortOrder.value === 'asc') {
            lap_kondisi_jembatans.value.sort((a, b) => parseFloat(a.panjang_ruas_jembatan) - parseFloat(b.panjang_ruas_jembatan));
            panjangSortOrder.value = 'desc';
        } else {
            lap_kondisi_jembatans.value.sort((a, b) => parseFloat(b.panjang_ruas_jembatan) - parseFloat(a.panjang_ruas_jembatan));
    ...
    const sortTahunPembangunan = () => {
        if (tahunPembangunanSortOrder.value === 'asc') {
            lap_kondisi_jembatans.value.sort((a, b) => parseInt(a.tahun_pembangunan) - parseInt(b.tahun_pembangunan));
            tahunPembangunanSortOrder.value = 'desc';
        } else {
            lap_kondisi_jembatans.value.sort((a, b) => parseInt(b.tahun_pembangunan) - parseInt(a.tahun_pembangunan));
    ...
    const sortProgress = () => {
        if (progressSortOrder.value === 'asc') {
            lap_kondisi_jembatans.value.sort((a, b) => parseInt(a.progress) - parseInt(b.progress));
            progressSortOrder.value = 'desc';
        } else {
            lap_kondisi_jembatans.value.sort((a, b) => parseInt(b.progress) - parseInt(a.progress));
    ...
    function exportToExcel() {
        ...
        // Get table element
        var table = document.getElementById("lap_kondisi_jembatanTable");
        ...
        XLSX.writeFile(wb, "list_lap_kondisi_jembatan.xlsx");
