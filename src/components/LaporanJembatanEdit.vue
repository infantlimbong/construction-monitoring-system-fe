<template>
    <div class="p-5">
        <h1 class="text-2xl mb-5">Edit Laporan Kegiatan Jembatan</h1>
        ...
        <form v-if="laporan_kegiatan" @submit.prevent="updateLaporanKegiatan" class="max-w-full mx-auto mt-5">
            ...
            <div class="mb-4">
                <label for="id_jembatan" class="font-bold block mb-1">Nama Ruas Jembatan</label>
                <div class="flex gap-2">
                    <select v-model="laporan_kegiatan.id_jembatan" id="id_jembatan" class="shadow border rounded w-full py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
                        <option value="">-</option>
                        <option v-for="lap_kondisi_jembatan in lapKondisiJembatans" :key="lap_kondisi_jembatan.id" :value="lap_kondisi_jembatan.id">{{ lap_kondisi_jembatan.nama_ruas_jembatan }}</option>
                    </select>
                </div>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input v-model="laporan_kegiatan.lokasi" name="lokasi" id="lokasi" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea v-model="laporan_kegiatan.keterangan" rows="3" name="keterangan" id="keterangan" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline"></textarea>
            </div>
...
<script setup>
    ...
    const updateLaporanKegiatan = async () => {
        ...
        try {
            await axios.put(`http://localhost:8000/api/laporan_kegiatans/${laporan_kegiatan.value.id}`, {
                id_jembatan: laporan_kegiatan.value.id_jembatan,
    ...
    onMounted(async () => {
        try {
            // Fetch laporan_kegiatan, lap_kondisi_jembatans, and sumber_danas
            const [
                laporanResponse,
                jembatanResponse,
                danaResponse
                ...
                axios.get("http://localhost:8000/api/lap_kondisi_jembatans", {
                    headers: {
                        Authorization: `Bearer ${authStore.accessToken}`,
                    },
                }),
            ...
            lapKondisiJembatans.value = jembatanResponse.data.data;
