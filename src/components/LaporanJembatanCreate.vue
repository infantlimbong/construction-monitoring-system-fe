<template>
    <div class="p-5">
        ...
        <form @submit.prevent="createLaporanKegiatan" class="max-w-full mx-auto mt-5">
            ...
            <div class="mb-4">
                <label for="id_jembatan" class="font-bold block mb-1">Nama Jembatan</label>
                <div class="flex gap-2">
                    <select v-model="id_jembatan" id="id_jembatan" class="shadow border rounded w-2/3 py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
                        <option v-for="lap_kondisi_jembatan in filteredLapKondisiJembatans" :key="lap_kondisi_jembatan.id" :value="lap_kondisi_jembatan.id">{{ lap_kondisi_jembatan.nama_ruas_jembatan }}</option>
                    </select>
                    <input v-model="searchLapKondisiJembatan" name="searchLapKondisiJembatan" id="searchLapKondisiJembatan" @input="filterLapKondisiJembatan" placeholder="Cari Nama Ruas Jalan..." type="text" class="shadow appearance-none border rounded w-1/3 py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
                </div>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input v-model="lokasi" name="lokasi" id="lokasi" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input v-model="keterangan" name="keterangan" id="keterangan" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 bg-secondary-black text-white leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
...
<script setup>
    ...
    const searchLapKondisiJembatan = ref("");
    const searchDana = ref("");
    const lapKondisiJembatans = ref([]);
    const sumberDanas = ref([]);

    // Fetch lap_kondisi_jembatans on component mount
    onMounted(async () => {
        ...
        lapKondisiJembatans.value = response.data.data;
    });
    ...
    // Filter lap_kondisi_jembatans based on user input
    const filteredLapKondisiJembatans = ref([]);
    const filterLapKondisiJembatan = () => {
        filteredLapKondisiJembatans.value = lapKondisiJembatans.value.filter(lap_kondisi_jembatan =>
            lap_kondisi_jembatan.nama_ruas_jembatan.toLowerCase().includes(searchLapKondisiJembatan.value.toLowerCase())
        );
    };
    ...
    // Watch for changes in lap_kondisi_jembatans and update filteredLapKondisiJembatans accordingly etc.
    watch(lapKondisiJembatans, () => {
        filterLapKondisiJembatan();
    });
    ...
    // Fetch lap_kondisi_jembatan name based on selected ID
    const fetchLapKondisiJembatanName = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/api/lap_kondisi_jembatans/${id_jembatan.value}`, {
                headers: {
                    Authorization: `Bearer ${authStore.accessToken}`,
                },
            });
            kegiatan.value = response.data.nama_ruas_jembatan;
        } catch (error) {
            console.error("Error fetching lap_kondisi_jembatan name:", error);
            kegiatan.value = ""; // Clear lap_kondisi_jembatan name if error occurs
        }
    };

    // Watch for changes in id_jembatan and fetch lap_kondisi_jembatan name
    watch(id_jembatan, fetchLapKondisiJembatanName);
    ...
    const createLaporanKegiatan = async () => {
        ...
        if (!id_jembatan.value) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Laporan jembatan harus dipilih'
            });
            return;
        }
