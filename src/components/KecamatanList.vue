<template>
    <div class="px-5">
        <h1 class="text-3xl my-5">List Kecamatan</h1>
        
        <div class="flex flex-wrap gap-y-2 md:flex-nowrap md:justify-between">
            <div class="flex items-center gap-2">
                <router-link v-if="isAdmin || isKaryawan1" class="bg-blue-500 hover:bg-blue-700 text-md px-3 py-1 rounded transition-all duration-200" to="/createkecamatan">Tambah</router-link>
            ...
            <div class="flex items-center justify-between w-full md:justify-normal md:gap-2 md:w-min">
                <input type="text" v-model="searchQuery" name="searchQuery" placeholder="Cari kecamatan..." class="block py-1 px-3 border border-white rounded bg-main-black focus:outline-1">
        ...
        <div class="overflow-x-scroll">
            <table id="kecamatanTable" class="min-w-full divide-y mt-2 divide-white">
                <thead>
                ...
                <tbody class="divide-y divide-white" v-if="paginatedKecamatans.length > 0">
                    <tr v-for="(kecamatan, loop) in paginatedKecamatans" :key="kecamatan.id" class="odd:bg-main-black hover:bg-stone-950">
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ loop + 1 }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ kecamatan.nama_kecamatan }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ getKabupatenName(kecamatan.id_kabupaten) }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ formatDate(kecamatan.created_at) }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm">{{ formatDate(kecamatan.updated_at) }}</td>
                        <td v-if="isAdmin || isKaryawan1" class="px-4 py-2 whitespace-nowrap text-sm">
                            <router-link :to="`/editkecamatan/${kecamatan.id}`" class="text-profile-color hover:underline mr-4">Edit</router-link>
                            <button v-if="isAdmin" @click="deleteKecamatan(kecamatan)" class="text-red-500 hover:underline">Hapus</button>
                        </td>
                    </tr>
                </tbody>
                <tbody v-else>
...
<script setup>
    ...
    const totalPages = computed(() => Math.ceil(filteredKecamatans.value.length / entitiesPerPage.value));
    ...
    const fetchKecamatans = async () => {
        ...
        kecamatans.value = response.data.data.reverse();

        // Fetch and store kabupatens
        const kabupatenResponse = await axios.get("http://localhost:8000/api/kabupatens", {
            headers: {
                Authorization: `Bearer ${authStore.accessToken}`,
            },
        });
        const kabupatens = kabupatenResponse.data.data;
        for (const kabupaten of kabupatens) {
            kabupatensMap.value[kabupaten.id] = kabupaten.nama_kabupaten;
        }
    };

    onMounted(fetchKecamatans);

    // Computed property to filter kecamatans based on searchQuery
    const filteredKecamatans = computed(() => {
        return kecamatans.value.filter(kecamatan => {
            return kecamatan.nama_kecamatan.toLowerCase().includes(searchQuery.value.toLowerCase());
        });
    });

    // Computed property for paginated kecamatans
    const paginatedKecamatans = computed(() => {
        const startIndex = (currentPage.value - 1) * entitiesPerPage.value;
        const endIndex = startIndex + entitiesPerPage.value;
        return filteredKecamatans.value.slice(startIndex, endIndex);
    });

    // Sweet alert for delete
    const deleteKecamatan = async (kecamatan) => {
        // Show SweetAlert confirmation dialog
        const result = await Swal.fire({
            title: 'Konfirmasi Penghapusan',
            text: 'Menghapus kecamatan juga akan menghapus data Desa yang berhubungan. Konfirmasi?',
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`http://localhost:8000/api/kecamatans/${kecamatan.id}`, {
                    headers: {
                        Authorization: `Bearer ${authStore.accessToken}`,
                    },
                });
                ...
                Swal.fire(
                    'Gagal menghapus!',
                    'Terjadi kesalahan saat menghapus kecamatan.',
                    'error'
                );
            }
        }
    ...
    const getKabupatenName = (kabupatenId) => {
        return kabupatensMap.value[kabupatenId] || 'Unknown';
    };
    ...
    const sortByName = (type) => {
        if (type === 'kecamatan') {
            if (kecamatanSortOrder.value === 'asc') {
                kecamatans.value.sort((a, b) => a.nama_kecamatan.localeCompare(b.nama_kecamatan));
                kecamatanSortOrder.value = 'desc';
            } else {
                kecamatans.value.sort((a, b) => b.nama_kecamatan.localeCompare(a.nama_kecamatan));
                kecamatanSortOrder.value = 'asc';
            }
        } else if (type === 'kabupaten') {
            if (kabupatenSortOrder.value === 'asc') {
                kecamatans.value.sort((a, b) => {
                    const kabA = kabupatensMap.value[a.id_kabupaten] || 'Unknown';
                    const kabB = kabupatensMap.value[b.id_kabupaten] || 'Unknown';
    ...
    function exportToExcel() {
        ...
        // Get table element
        var table = document.getElementById("kecamatanTable");
        ...
        XLSX.writeFile(wb, "list_kecamatan.xlsx");
